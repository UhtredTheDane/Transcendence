...
export default class Game {
	...
	constructor(fieldValue) {
		...
		this._socket = new WebSocket("ws://" + window.location.host + "/ws/game/" + this._gameId + "/" + this._playerRole);
	...
	#initOnOpen() {
		this._socket.onopen = () => {
			console.log("Connect� au WebSocket du jeu");
	...
	#initOnMessage() {
		this._socket.onmessage = function (event) {
			...
			if (data.type === "update_position") {
				...
				//alert(playerRole == getWinner() ? "Vous avez gagn� !" : "Vous avez perdu !");
				//setTimeout(() => {
	...
	endGame() {
		...
		document.getElementById("pauseButton").display = "none";
		alert(playerRole == this._field.getWinner() ? "Vous avez gagn� !" : "Vous avez perdu !");
		window.location.href = "/";
	}
